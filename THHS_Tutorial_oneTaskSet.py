...
######## Apr 26
## edits for one task set version
...
def pracBlocks(n_trials=n_trials):
    ...
    Extra_dimension_switch_probability = 0.0
    Intra_dimension_switch_probability = 1
    Cue_time = .5
    Pic_time = 3
...
#    event.waitKeys()
    ...
    Quiz_txt.draw()
    blue_sq.pos=(-0.7*vis_deg_poly,0)#(-4,0) #blue FILLED square
    blue_sq.draw()
    Equals.draw()
    Face_Stim.draw()
    win.flip()
