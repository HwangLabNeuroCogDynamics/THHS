...
### TASK SET MAPPINGS SWAPPED
## Filled-->Color, Donut-->Shape
## Yes==0 (2), No==1 (1)
"""
...
if behavFlag:
    ...
    ShapeMatters = visual.TextStim(win=win, name='ShapeMatters',
        text=u'If FILLED, then COLOR matters!',
        alignVert='center', units='norm',
    ...
    Instruct_new = visual.TextStim(win=win, name='Instruct_new',
        text=u'Before, FILLED = COLOR, now EMPTY = SHAPE',
        alignVert='center', units='norm',
    ...
    ColorMatters = visual.TextStim(win=win, name='ColorMatters',
        text=u'If EMPTY, then SHAPE matters!',
        alignVert='center', units='norm',
...
#### Here we reinforce the first interdimensional shift rules
    ...
    ShapeMatters.draw()
    red_sq=copy.copy(polygon_filled) # Filled Blue Square
    red_sq.setColor('red')
    red_sq.pos=(-0.5*vis_deg_poly,0) # -4,0
    red_sq.size=(vis_deg_poly+2,vis_deg_poly+2)
    red_circ=copy.copy(circle_filled) # Filled Blue Circle
    red_circ.setColor('red')
    red_circ.pos=(0.5*vis_deg_circ,0) #2,0
    #blue_circ.size=(vis_deg_circ+2,vis_deg_circ+2)
    #Arrow.height=(.2)
    Arrow.draw()
    red_circ.draw()
    red_sq.draw()
...
#### indicates that an interdimensional shift has occurred between Square and Circle
    blue_sq=copy.copy(polygon_filled)
    blue_sq.setColor('blue')
    blue_sq.pos=(-0.5*vis_deg_poly,0) #(-0.5*circ_ratio*vis_deg_circ,0)
    blue_sq.draw() # Filled Blue circle
    Equals.draw()
    Scene_txt.pos=(.6,0)
    Scene_txt.draw()
    win.flip()
...
#### Filled Polygon (square) = Face, Filled Circle = Scene
    ...
    blue_circ.size=(vis_deg_circ*.6,vis_deg_circ*.6)
    red_circ.size=(vis_deg_circ*.6,vis_deg_circ*.6)
    blue_circ.pos=(-.5*circ_ratio*vis_deg_circ,-.30*circ_ratio*vis_deg_circ) #(-3.5,2)
...
#### Does the participant now know what the second answer will be?
    red_circ.pos=(-.5*vis_deg_poly,.30*vis_deg_poly)#(-3.5,2)
    blue_circ.pos=(-.5*circ_ratio*vis_deg_circ,-.30*circ_ratio*vis_deg_circ)
    #ShapeMatters.pos=(0,5.5)
    #Equals_1.pos=(.25,.35)
    #Equals_2.pos=(.25,-.35)
    Scene_txt.pos=(.55,-.35)
...
#### will be a face or a scene
    sq_R_do.lineColor=('red')
    sq_R_do.pos=(-.5*circ_ratio*vis_deg_circ,0)
    sq_R_do.size=(vis_deg_circ-1,vis_deg_circ-1)
    Arrow.height=(.2)
    Face_txt.pos=(.35,0)
    Equals.draw()
    Face_txt.draw()
...
#### no longer matters
    circ_R_do.lineColor=('red')
    circ_R_do.pos=(-.5*circ_ratio*vis_deg_circ,0)#(-7,0)
    circ_R_do.size=(vis_deg_circ-1,vis_deg_circ-1)
    Equals.draw()
    Scene_txt.pos=(.35,0)
    Scene_txt.draw()
...
#### to red and blue is?
    ...
    circ_B_do.pos=(-.25*circ_ratio*vis_deg_circ,-0.35*circ_ratio*vis_deg_circ )#(-2,-4)
    circ_B_do.size=(vis_deg_circ,vis_deg_circ)
    sq_B_do.pos=(-.25*vis_deg_poly,0.35*vis_deg_poly)#(-2,4)
    Arrow.height=(.2)
    #Equals_1.pos=(.25,.4)
    #Equals_2.pos=(.25,-.4)
    Equals_1.draw()
    ...
    circ_B_do.draw() # Empty Red Square
    win.flip()
    event.waitKeys()
    ...
    Quiz_txt.draw()
    circ_R_do.pos=(-0.5*circ_ratio*vis_deg_circ,0) #red EMPTY circle
    circ_R_do.draw()
    Equals.pos=(0,0)
    Equals.draw()
