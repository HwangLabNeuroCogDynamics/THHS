...
## August 22, 2019: ONE TASK SET VERSION: for patients
## August 21, 2019: Expanding time allotted for the cue and pic presentation. Using pic-time in while loop for probe period
## August 15, 2019: Re-structuring how the csv if being generated, and updated to construct log trial by trial
## TRY THIS ONE DAVID 04/16/19 :)
...
from psychopy import  gui, visual, core, data, event, clock,parallel, logging
...
expInfo = {'block': '001', 'participant': '', 'Stay_R':'0.5'}#,'IDS_R':'0.5'} 
...
Number_of_IDS = round(Total_switch_trials *  Switch_probability)
...
Trial_order = np.concatenate(( np.repeat(Stay_trial, Number_of_stay), np.repeat(Intra_dimension_switch, Number_of_IDS)))
...
filename = thisDir_save + u'/%s_%s_%s_%s' % (expInfo['participant'], expInfo['block'],expName, expInfo['date'])
...
#EEGflag=0
if EEGflag:
    port=parallel.ParallelPort(address=53504)#serial.Serial('COM4',baudrate=115200)
    port.setData(0)
    startSaveflag=254 #bytes([254])
    stopSaveflag=255
    
    cuetrigDict={'IDS_trig':101,'Stay_trig':105}
    face_probe_trig=107
    scene_probe_trig=109
    subNonResp_trig=111
    subResp_trig=113
    ITI_trig=115
    endofBlock_trig=117
...
#This will be for CSV initiationfile
...
our_data=logging.LogFile(filename+'.csv',logging.DATA,'a')

def makeCSV(thistrialDict,trials):

    if EEGflag:
        header_list=['sub','block','trial_n','cue','Task','pic','Trial_type','trial_Corr','rt','What_Is_CorrResp','Subject_Respo','img_path','trigs']
    else:
        header_list=['sub','block','trial_n','cue','Task','pic','Trial_type','trial_Corr','rt','What_Is_CorrResp','Subject_Respo','img_path']
    
    if trials==0: # if this is the first trial, write out the header info
        for header_name in header_list:
            our_data.write(header_name+',')
        our_data.write('\n')
            
        
    for header in header_list:
        our_data.write('{0},'.format(thistrialDict[trials][header]))
        if header == header_list[-1]: #if this is the last element, add a new line
            our_data.write('\n')
    

#    for header_name in header_list:
#        our_data.write(header_name+',')  #write out header info first-- this will be the keys of trial_dict[trial_num], where trial_num is the last trial but all keys should be the same for every trial
#    our_data.write('\n')
#    for trial in thistrialDict.keys():
#        for header in header_list:
#            our_data.write('{0},'.format(thistrialDict[trial][header]))
#            if header == header_list[-1]: #if this is the last element, add a new line
#                our_data.write('\n')
...
#            corr_resp=no_key
    ...
    if EEGflag:
        win.callOnFlip(port.setData,cue_trig)
    ...
    if EEGflag:
        win.callOnFlip(port.setData,stim_trig)
    
    #Trial_dict[trial_num]['pic_stim'].draw()
    
    Trial_dict[trial_num]['pic_stim'].autoDraw=True
    event.clearEvents()
    max_win_count=int(Pic_time/(1/refresh_rate))
    win_count=0
    subRespo=None
    RT_clock.reset()
    while not subRespo:
        win.flip()
        subRespo=event.getKeys(timeStamped=RT_clock, keyList=[yes_key,no_key,'escape'])
    ...
    if EEGflag:
        win.callOnFlip(port.setData,ITI_trig)

    # draw the ITI fixation
    #Fix_Cue.draw()
    wait_here(randomchoice(ITIs),to_draw=Fix_Cue)
     # randomly pick one from the range of ITIs
    makeCSV(thistrialDict=Trial_dict,trials=trial_num)



if EEGflag:
    port.setData(stopSaveflag)