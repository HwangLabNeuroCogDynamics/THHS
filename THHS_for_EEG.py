...
expInfo = {'block': '001', 'participant': '', 'Stay_R':'0.5','IDS_R':'0.5','refresh_rate':50} 
# block = block #
# participant = subject ID
# Stay_R = the ratio of stay trials from the TOTAL number of trials
# IDS_R = the ratio of IDS trials from the SWITCH trials (where switch trials = number of total trials - number of stay trials)
# EDS_R is implicitly defined by [(1-IDS_R)*number of switch trials]
...
real_Total_trials = 83  # Adjust number as indicated for experiment
Number_of_initial_stays = 3 # there are 3 initial stay trials
Total_trials=real_Total_trials-Number_of_initial_stays # We always insert 3 Stay blocks at the beginning of each script, so the total_trials is seperated from real_Total_trials
                                    # in order to randomize the conditions of the rest of the trials after the inital 3 Stay trials 
Stay_probability=float(expInfo['Stay_R'])
Switch_probability =1-float(expInfo['Stay_R'])
Extra_dimension_switch_probability = 1-float(expInfo['IDS_R']) # we specify the IDS ratio when we intialize the script, and the EDS ratio is just [1-IDS ratio]
Intra_dimension_switch_probability = float(expInfo['IDS_R'])
if Extra_dimension_switch_probability + Intra_dimension_switch_probability > 1: # making sure the IDS/EDS ratio make sense, ie that it doesn't add up to more than 1
...
if Switch_probability >= .9: # same w switch ratio
    ...
    core.quit()
Cue_time = .5 # 500 ms of cue
Initial_wait_time = 3 # the beginning of the script is offset by 3 seconds
...
EEGflag=1
yes_key='1' # response keys, where yes_key = "Yes" to the cue'd question
no_key='0'


refresh_rate=expInfo['refresh_rate']

#EEGflag=0 # uncomment this for debugging
if EEGflag:
...
def makeCSV(filename, thistrialDict, trial_num):
    with open(filename + '.csv', mode='w') as our_data:
         ExpHead=thistrialDict[trial_num].keys() # constructing header information from the dictionary keys
...
# the stimuli in this script are in 'norm' units, 
# these variables specify the size of circles and squares,
# where the square has to be made a liiittle bigger in
# order to look the same size as the circle
...
for trial_num in range(len(Trial_dict)): #
    ...
    win_count=0
    subRespo=None
    RT_clock.reset() # reset RT clock to start at zero
    while not subRespo:
        win.flip()
    ...
    makeCSV(filename=filename,thistrialDict=Trial_dict,trial_num=trial_num) # saving CSV at every trial 
